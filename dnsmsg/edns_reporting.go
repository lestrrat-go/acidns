...
func NewReportChannel(agent dnsname.Name) EDNSOption {
...
func ReportChannelAgent(o EDNSOption) (dnsname.Name, bool) {
	if o.Code() != EDNSOptionReportChannel {
		return dnsname.Name{}, false
	}
	u := wire.NewUnpacker(o.Data())
	n, err := u.Name()
	if err != nil {
		return dnsname.Name{}, false
	}
...
func BuildErrorReportName(qname dnsname.Name, qtype rrtype.Type, infoCode ExtendedErrorCode, agent dnsname.Name) (dnsname.Name, error) {
	if !qname.IsValid() || qname.IsRoot() {
		return dnsname.Name{}, fmt.Errorf("%w: error-report qname must be non-root", ErrInvalidMessage)
	}
	if !agent.IsValid() {
		return dnsname.Name{}, fmt.Errorf("%w: error-report agent invalid", ErrInvalidMessage)
	}
	parts := []string{
		"_er",

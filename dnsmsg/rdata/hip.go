...
type HIP interface {
	...
	HIT() []byte
	PublicKey() []byte
	RendezvousServers() []dnsname.Name
}
...
func (h hip) RendezvousServers() []dnsname.Name { return h.rvs }
func (h hip) Pack(p *wire.Packer) {
...
func NewHIP(alg HIPAlgorithm, hit, pubkey []byte, rvs ...dnsname.Name) (HIP, error) {
	if len(hit) > 0xff {
	...
	copy(pcp, pubkey)
	rcp := make([]dnsname.Name, len(rvs))
	copy(rcp, rvs)
	return hip{alg: alg, hit: hcp, pubkey: pcp, rvs: rcp}, nil
}

func unpackHIP(u *wire.Unpacker, rdlen int) (HIP, error) {
	end := u.Off() + rdlen
	hitLen, err := u.Uint8()
	if err != nil {
		return nil, err

...
func TestSVCBRoundTrip(t *testing.T) {
	...
	r := rdata.NewSVCB(1, dnsname.MustParse("svc.example.com"),
...
func TestHTTPSType(t *testing.T) {
	t.Parallel()
	r := rdata.NewHTTPS(1, dnsname.MustParse("example.com"))
...
func TestSVCBWire(t *testing.T) {
	t.Parallel()
	r := rdata.NewSVCB(1, dnsname.MustParse("foo.example.com"),
		rdata.NewSVCBParam(rdata.SvcParamPort, []byte{0x01, 0xbb}),
	)

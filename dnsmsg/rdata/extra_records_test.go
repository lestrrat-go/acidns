...
func TestIPSECKEYName(t *testing.T) {
	t.Parallel()
	pk := []byte{0xaa, 0xbb}
	r := rdata.NewIPSECKEYName(5, rdata.IPSECKEYAlgECDSA, dnsname.MustParse("gw.example.com"), pk)
	got := packUnpack(t, r).(rdata.IPSECKEY)
	require.Equal(t, rdata.IPSECKEYGatewayName, got.GatewayType())
	require.True(t, got.GatewayName().Equal(dnsname.MustParse("gw.example.com")))
	require.Equal(t, pk, got.PublicKey())
}
...
func TestHIP(t *testing.T) {
	...
	r, err := rdata.NewHIP(rdata.HIPAlgRSA, hit, pk, dnsname.MustParse("rvs.example.com"))
	require.NoError(t, err)
...
func TestLP(t *testing.T) {
	t.Parallel()
	r := rdata.NewLP(40, dnsname.MustParse("l64.example.com"))
	got := packUnpack(t, r).(rdata.LP)
	require.True(t, got.FQDN().Equal(dnsname.MustParse("l64.example.com")))
}

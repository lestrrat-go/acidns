...
func (m mx) Exchange() dnsname.Name { return m.exchange }
func (m mx) Pack(p *wire.Packer) {
	p.Uint16(m.pref)
	p.Name(m.exchange)
}

// NewMX returns an MX rdata.
func NewMX(pref uint16, exchange dnsname.Name) MX {
	return mx{pref: pref, exchange: exchange}
}

func unpackMX(u *wire.Unpacker) (MX, error) {
	pref, err := u.Uint16()
	if err != nil {
		return nil, err
	}

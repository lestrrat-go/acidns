...
func TestRRSIGAccessors(t *testing.T) {
	...
	r := rdata.NewRRSIG(rrtype.A, rdata.AlgRSASHA256, 3, time.Hour,
		exp, inc, 1234, dnsname.MustParse("example.com"), []byte{0xab})
...
func TestNSEC3PARAMAccessors(t *testing.T) {
	t.Parallel()
	p := rdata.NewNSEC3PARAM(1, 0, 100, []byte{0xca, 0xfe})
...
func TestNSEC3Accessors(t *testing.T) {
	t.Parallel()
	n := rdata.NewNSEC3(1, 2, 3, []byte{0x01}, []byte{0x02}, []rrtype.Type{rrtype.A})
...
func TestILNPPreferenceAccessors(t *testing.T) {
	...
	require.Equal(t, uint16(40), rdata.NewLP(40, dnsname.MustParse("a.b")).Preference())
}
...
func TestNAPTRAccessors(t *testing.T) {
	t.Parallel()
	n, err := rdata.NewNAPTR(1, 2, "U", "E2U+sip", "!^.*$!sip:!", dnsname.MustParse("example.com"))
	require.NoError(t, err)
	require.Equal(t, "!^.*$!sip:!", n.Regexp())
	require.True(t, n.Replacement().Equal(dnsname.MustParse("example.com")))
}
...
func TestSVCBPropertiesAccessors(t *testing.T) {
	t.Parallel()
	s := rdata.NewSVCB(1, dnsname.MustParse("example.com"))
...
func TestNSECRoundTrip(t *testing.T) {
	t.Parallel()
	n := rdata.NewNSEC(dnsname.MustParse("next.example."), []rrtype.Type{rrtype.A, rrtype.RRSIG, rrtype.NSEC})

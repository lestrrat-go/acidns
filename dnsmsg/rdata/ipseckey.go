...
func (k ipseckey) GatewayName() dnsname.Name        { return k.gwName }
func (k ipseckey) PublicKey() []byte                { return k.pubkey }
func (k ipseckey) Pack(p *wire.Packer) {
	p.Uint8(k.prec)
	p.Uint8(uint8(k.gt))
	p.Uint8(uint8(k.alg))
	switch k.gt {
...
func NewIPSECKEYName(prec uint8, alg IPSECKEYAlgorithm, name dnsname.Name, pubkey []byte) IPSECKEY {
	cp := make([]byte, len(pubkey))
	copy(cp, pubkey)
	return ipseckey{prec: prec, gt: IPSECKEYGatewayName, alg: alg, gwName: name, pubkey: cp}
}

func unpackIPSECKEY(u *wire.Unpacker, rdlen int) (IPSECKEY, error) {
	end := u.Off() + rdlen
	prec, err := u.Uint8()
	if err != nil {
		return nil, err

...
func (s *svcb) Target() dnsname.Name { return s.target }
func (s *svcb) Params() []SVCBParam  { return s.params }

func (s *svcb) Pack(p *wire.Packer) {
	p.Uint16(s.priority)
...
func NewSVCB(priority uint16, target dnsname.Name, params ...SVCBParam) SVCB {
	return &svcb{
		rrType:   rrtype.SVCB,
		priority: priority,
		target:   target,
...
func NewHTTPS(priority uint16, target dnsname.Name, params ...SVCBParam) SVCB {
	return &svcb{
		rrType:   rrtype.HTTPS,
		priority: priority,
		target:   target,
...
func unpackSVCB(t rrtype.Type, u *wire.Unpacker, rdlen int) (SVCB, error) {
	end := u.Off() + rdlen
	prio, err := u.Uint16()
	if err != nil {
		return nil, err

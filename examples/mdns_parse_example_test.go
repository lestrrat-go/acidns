...
func Example_mdns_parse() {
	...
	resp, _ := dnsmsg.NewBuilder().
		...
		Answer(dnsmsg.NewRecord(instance, time.Minute, rdata.NewSRV(0, 0, 80, host))).
		Answer(dnsmsg.NewRecord(instance, time.Minute, txt)).
		Additional(dnsmsg.NewRecord(host, time.Minute, rdata.NewA(netip.MustParseAddr("192.0.2.50")))).
		Build()

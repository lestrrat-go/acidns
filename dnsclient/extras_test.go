...
func TestRCodeErrorIs(t *testing.T) {
	t.Parallel()
	live := &dnsclient.RCodeError{Code: dnsmsg.RCODENXDomain}
...
func (s *stubExchanger) Exchange(_ context.Context, _ dnsmsg.Message) (dnsmsg.Message, error) {
	return s.resp, s.err
}
...
func TestOptionAccessors(t *testing.T) {
	...
	_ = dnsclient.WithSearchList(dnsname.MustParse("example.com"))
...
func TestAnswerMethods(t *testing.T) {
	...
	require.NoError(t, err)
	ans, err := r.Resolve(context.Background(), dnsname.MustParse("example.com"), rrtype.A)
	require.NoError(t, err)

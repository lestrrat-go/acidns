...
func TestParse(t *testing.T) {
	...
	t.Run("simple", func(t *testing.T) {
		n, err := dnsname.Parse("example.com")
	...
	t.Run("case folded", func(t *testing.T) {
		a, err := dnsname.Parse("EXAMPLE.COM")
		require.NoError(t, err)
		b, err := dnsname.Parse("example.com")
		require.NoError(t, err)
	...
	t.Run("subdomain", func(t *testing.T) {
		n, err := dnsname.Parse("a.b.c.example.com")
		require.NoError(t, err)
		require.Equal(t, 5, n.NumLabels())
	})

	t.Run("escaped dot", func(t *testing.T) {
		n, err := dnsname.Parse(`weird\.label.example.com`)
		require.NoError(t, err)
	...
	t.Run("escaped decimal", func(t *testing.T) {
		n, err := dnsname.Parse(`\032space.example.com`)
		require.NoError(t, err)
		labels := slices.Collect(n.Labels())
	...
	t.Run("empty label", func(t *testing.T) {
		_, err := dnsname.Parse("foo..bar")
		require.Error(t, err)
	})

	t.Run("label too long", func(t *testing.T) {
		_, err := dnsname.Parse(strings.Repeat("a", 64) + ".example.com")
		require.Error(t, err)
	})
...
func TestEqual(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.a+"_vs_"+tc.b, func(t *testing.T) {
			a, err := dnsname.Parse(tc.a)
			require.NoError(t, err)
			b, err := dnsname.Parse(tc.b)
			require.NoError(t, err)
...
func TestParent(t *testing.T) {
	...
	n := dnsname.MustParse("a.b.example.com")
...
func TestFromLabels(t *testing.T) {
	...
	n, err := dnsname.FromLabels("www", "example", "com")
	require.NoError(t, err)
	require.Equal(t, "www.example.com.", n.String())

	n, err = dnsname.FromLabels()
	require.NoError(t, err)
	require.True(t, n.IsRoot())

	_, err = dnsname.FromLabels("", "example")
	require.Error(t, err)

	_, err = dnsname.FromLabels(strings.Repeat("a", 64))
	require.Error(t, err)
}
...
func TestDecodeWire(t *testing.T) {
	...
	t.Run("simple", func(t *testing.T) {
		...
		n, off, err := dnsname.DecodeWire(buf, 0)
		require.NoError(t, err)
		require.Equal(t, "example.com.", n.String())
	...
	t.Run("compression pointer", func(t *testing.T) {
		...
		n, off, err := dnsname.DecodeWire(buf, 13)
		require.NoError(t, err)
		require.Equal(t, "www.example.com.", n.String())
	...
	t.Run("pointer loop", func(t *testing.T) {
		...
		_, _, err := dnsname.DecodeWire(buf, 0)
		require.Error(t, err)
	})
...
func TestRoundTrip(t *testing.T) {
	...
	for _, s := range names {
		t.Run(s, func(t *testing.T) {
			n := dnsname.MustParse(s)
			buf := n.AppendWire(nil)
			n2, _, err := dnsname.DecodeWire(buf, 0)
			require.NoError(t, err)

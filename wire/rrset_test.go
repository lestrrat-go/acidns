...
func TestRRset(t *testing.T) {
	...
	a1 := wire.NewRecord(name, 60*time.Second, rdata.NewA(netip.MustParseAddr("192.0.2.1")))
	a2 := wire.NewRecord(name, 30*time.Second, rdata.NewA(netip.MustParseAddr("192.0.2.2")))
...
func TestRRsetMixedTypeRejected(t *testing.T) {
	t.Parallel()
	name := wirebb.MustParse("example.com")
	a := wire.NewRecord(name, 60*time.Second, rdata.NewA(netip.MustParseAddr("192.0.2.1")))
	aaaa := wire.NewRecord(name, 60*time.Second, rdata.NewAAAA(netip.MustParseAddr("2001:db8::1")))
...
func TestGroupRecords(t *testing.T) {
	...
	a := wire.NewRecord(name, 60*time.Second, rdata.NewA(netip.MustParseAddr("192.0.2.1")))
	aaaa := wire.NewRecord(name, 60*time.Second, rdata.NewAAAA(netip.MustParseAddr("2001:db8::1")))
	a2 := wire.NewRecord(name, 60*time.Second, rdata.NewA(netip.MustParseAddr("192.0.2.2")))

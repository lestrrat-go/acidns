...
func TestRRSIG(t *testing.T) {
	...
	r := rdata.NewRRSIG(rrtype.A, rdata.AlgED25519, 2,
		...
		wirebb.MustParse("example.com"), []byte{0xaa, 0xbb, 0xcc})
	...
	require.True(t, got.SignerName().Equal(wirebb.MustParse("example.com")))
...
func TestNSEC(t *testing.T) {
	...
	r := rdata.NewNSEC(wirebb.MustParse("next.example.com"), types)

	got := packUnpack(t, r).(rdata.NSEC)
	require.True(t, got.NextDomainName().Equal(wirebb.MustParse("next.example.com")))

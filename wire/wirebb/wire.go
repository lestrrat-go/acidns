// Package wirebb is the low-level DNS wire codec — pure-function primitives
// for byte-level packing/unpacking of names, integers, and length-prefixed
// strings. It owns the [Name] type because [Packer]/[Unpacker] need it; the
// parent [github.com/lestrrat-go/acidns/wire] package re-exports it as
// `wire.Name` (with friendlier constructor names like `wire.ParseName`).
package wirebb
...
func (p *Packer) NameUncompressed(n Name) {
	p.buf = n.AppendWire(p.buf)
}

func nameWire(n Name) string {
	if !n.IsValid() {
		return "\x00"
	}
